...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (profileTrigger && profileDropdown) {
        profileTrigger.addEventListener('click', (e) => {
            ...
            if (typeof Auth !== 'undefined' && Auth.isLoggedIn()) {
                ...
                const dashboardLink = role === 'admin' ? 'admin-dashboard.html' : 'user-dashboard.html';
                profileDropdown.innerHTML = `
                    <div class="dropdown-user-info" style="padding: 10px 15px; border-bottom: 1px solid rgba(255,255,255,0.05); margin-bottom: 5px;">
                        <p style="font-size: 0.8rem; font-weight: 600; color: var(--accent-color);">${role.toUpperCase()}</p>
                    </div>
    ...
    if (!document.getElementById('mobileAccountPanel')) {
        ...
        if (typeof Auth !== 'undefined' && Auth.isLoggedIn()) {
            ...
            const dashboardLink = role === 'admin' ? 'admin-dashboard.html' : 'user-dashboard.html';
            accountContent = `
                <p>Welcome back! Access your personalized control center below.</p>
                <div class="account-actions" style="display: flex; flex-direction: column; gap: 10px;">
                    <a href="${dashboardLink}" class="btn">GO TO DASHBOARD</a>
                    <a href="#" onclick="Auth.logout()" class="btn-outline">LOGOUT</a>
                </div>
            `;
        }
